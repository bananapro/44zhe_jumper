...
class CronController extends AppController {
	...
	function updateUserFanli($type = 'fanli') {
		...
		if ($type == 'fanli') {
			...
			//新建用户必须满3天以上才需要同步资产
			$users = $this->UserFanli->findAll("(role IN(1,2) AND status IN(1,3) AND created < '{$daydiff}') OR (status = 2 AND pause_date > '{$weekdate}') OR role = 4");
	...
	function updateMizheOrder($curl, $content = '') {
		...
		else $i = $curl->get('http://i.mizhe.com/order/income.html');
		...
		if ($i) {
			...
			foreach ($new as $n) {
				...
				if ($this->OrderFanli->find(array('did' => $n['did']))) continue;
...
ETO;
		...
		//计算change status
		$orders = $this->OrderFanli->findAll(array('status' => array(0, 3, 4)));
	 ...
	 * 3、修改Model/PayAlipayJifenModel.class.php trade 加入 $reviewMode = 0 防止返利升级用户等级代码
	 * 4、修改提现目标的支付宝
	 * 5、依次请求该方法生成的语句
	 */
