...
class StatController extends AppController {
	...
	function dataJump() {
		...
		$datas = $this->StatJump->query("SELECT p_price,p_fanli,count(*) as nu,DATE(created) as created FROM stat_jump WHERE DATE(created)>'$last_date' AND DATE(created) <> '2013-11-11' AND DATE(created) <> '2013-12-12' GROUP BY DATE(created), p_title");
		...
		foreach ($datas as $data) {
			...
			@$new_datas['all'][$data['created']] += 1;
		}

		if ($senior) {
			$datas = $this->StatJump->query("SELECT p_price, p_fanli ,count(*) as nu,DATE(created) as created FROM stat_jump WHERE jumper_type <> 'fanli' AND DATE(created)>'$last_date' AND DATE(created) <> '2013-11-11' AND DATE(created) <> '2013-12-12' GROUP BY DATE(created), p_title");

			clearTableName($datas);
			foreach ($datas as $data) {
				@$new_datas['other'][$data['created']] += 1;
			}
